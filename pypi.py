...
def get_package_metadata(package_index, package):
    u = urlparse(f"https://{package_index}/pypi/{package}/json")
...
def get_package_metadata_or_die(package_index, package):
    """Return a dict holding the metadata for a certain package."""
    resp = get_package_metadata(package_index, package)
    if resp.status != 200:
